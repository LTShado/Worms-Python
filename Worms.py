...
while launched:
    ...
    death_render = death_font.render(f"youre dead", True, traj_color)
    ...
    if status_death == 1:
        surface.blit(death_render, [100, 100])
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            while not bullet.collidelistall(ground) and bullet.x < 1260 and bullet.y < 940 and bullet.x>0 and bullet.y>0:
                time.sleep(.05)
                surface.fill(ciel)
            ...
            if bullet[0]+50 >= playerOne.body.x and bullet[0]-50 <= playerOne.body.x:
                status_death=1